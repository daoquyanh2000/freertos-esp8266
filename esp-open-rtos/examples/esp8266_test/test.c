...
void jsonCreateTask(void *pvParameter)
{
    ...
    while (1)
    {
        ...
        cJSON_AddBoolToObject(root, "r1", gpio_read(relay1_gpio) == 1 ? 1 : 0);
        cJSON_AddBoolToObject(root, "r2", gpio_read(relay2_gpio) == 1 ? 1 : 0);
        cJSON_AddBoolToObject(root, "r3", gpio_read(relay3_gpio) == 1 ? 1 : 0);
        cJSON_AddBoolToObject(root, "r4", gpio_read(relay4_gpio) == 1 ? 1 : 0);
